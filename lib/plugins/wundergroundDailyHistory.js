...
var Plugin = function (config, log) {
    ...
    function wundergroundCsvFetch(req, reply) {
        try {
            ...
            log.error(err, 'Error making request to the wundergrounddaily backend');
    ...
    function buildUriForStationRequest(stationId) {
        return util.format(config.wundergrounddaily.dailyuri, config.wundergrounddaily.server, stationId);
    }
};
